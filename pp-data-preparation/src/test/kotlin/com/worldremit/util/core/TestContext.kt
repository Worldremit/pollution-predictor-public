...
import com.worldremit.config.CustomTimestampExtractor
...
class TestContext(topology: Topology) {
    ...
    fun verifyAll(topic: TopicName, block: (List<TestRecord<SpecificRecord, SpecificRecord>>) -> Unit) {
        block.invoke(outputTopics.getOrThrow(topic).readRecordsToList())
    }
    fun verifyNoRecords(topic: TopicName) {
        outputTopics.getOrThrow(topic).also {
            assertThat(it.isEmpty)
                .withFailMessage("Topic ${topic.topicName} is not empty")
                .isTrue()
        }
    }
    ...
    private fun initInputTopics() = Topic.values().associate { topic: TopicName ->
        topic to testDriver.createInputTopic(topic.topicName, SERDE_KEY, SERDE_VALUE)
    }

    private fun initOutputTopics() = Topic.values().associate { topic: TopicName ->
        topic to testDriver.createOutputTopic(topic.topicName, SERDE_KEY, SERDE_VALUE)
    }
